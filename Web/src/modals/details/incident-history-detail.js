...
const IncidentHistoryDetail = () => {
    const [state, setState] = useContext(AppContext);
    const [incidentHistory, setIncidentHistory] = useState({});
    const [incident, setIncident] = useState({});

    useEffect(() => {
        setIncidentHistory(state.detailPayload);
    }, []);

    useEffect(() => {
        if (incidentHistory.incidentId !== undefined) {
            getIncident();
        }
    }, [incidentHistory]);
    ...
    const getIncident = useCallback(() => {
        ...
        IncidentsService.getById(incidentHistory.incidentId)
            .then(result => {
                setIncident(result);
...
export default IncidentHistoryDetail;